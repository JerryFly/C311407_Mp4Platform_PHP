...
<?php
/*
 * POST參數 MUID VideoID videoFileName
 */
...
if ($_FILES ["file"] ["error"] > 0) {
	...
	try {
		...
		$isOK = copy ( rootPATH . "/no_picture_available.jpg", rootPATH . "/" . $postInfo->photoAbsolutePath . "/" . $postInfo->photoFileName );
		$ajMessage->src = $postInfo->vodUrl . "/" . $postInfo->videoAbsolutePath . "/" . $postInfo->videoFileName;
		$ajMessage->result = true;
		$ajMessage->message = rootPATH . "/" . $postInfo->photoAbsolutePath . "/" . $postInfo->photoFileName;
	} catch ( Exception $e ) {
		$ajMessage->message = $e->getMessage ();
		$ajMessage->result = false;
	}
}
